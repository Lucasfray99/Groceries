...
final class WatchCloudKitShoppingListSync {
    static let shared = WatchCloudKitShoppingListSync()
    ...
    private func seedCloudFromLocalStoreIfNeeded() {
        guard let localData = UserDefaults.standard.data(forKey: WatchShoppingListStore.storageKey) else { return }
        save(localData)
    }

    private func applyCloudPayload(_ payload: Data) {
        DispatchQueue.main.async {
            guard UserDefaults.standard.data(forKey: WatchShoppingListStore.storageKey) != payload,
                  self.shouldApplyCloudPayload(payload),
                  let shoppingListData = try? JSONDecoder().decode(ShoppingListData.self, from: payload) else { return }

            WatchShoppingListStore.save(shoppingListData, syncsToCloudKit: false)
        }
    }
    ...
    private func shouldApplyCloudPayload(_ payload: Data) -> Bool {
        ...
        guard let localPayload = UserDefaults.standard.data(forKey: WatchShoppingListStore.storageKey),
